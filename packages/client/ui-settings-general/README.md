...
In the packaged Desktop application, the same navigation contains MCP Marketplace, Account, and Computer Control pages. They are absent from the regular browser application because its renderer has no desktop bridge. The pages use the compact Settings layout on both macOS and Windows rather than creating separate operating-system menu entries.
